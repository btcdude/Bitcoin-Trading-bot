 ...
 */
public class ANXapi extends MarketApi {
    private String baseurl = "https://anx.hk/gapiv2/";
    private String tickerurl = baseurl + "BTCUSD/money/ticker";
    ...
    public ANXapi(String initApiKey, String initApiSecret) {
        apikey = initApiKey;
    ...
    public void update_get_open_orders() {
        int count = 0;
        try {
            JSONObject jo = new JSONObject(query("BTCUSD/money/orders", new HashMap<String, String>(),2));
    ...
    public void submit_order(String type, String amount_int, String price_int) {
        ...
        JSONObject reponse;
        try {
            reponse = new JSONObject(query("BTCUSD/money/order/add", args));
            if (reponse.has("result") && reponse.getString("result").equals("success")) {
                settings.lasttrade.setOrderid(reponse.getString("data"));
            }
        } catch (JSONException e) {
    ...
    public void UpdateUSDFunds() {
        try {
            JSONObject jo = new JSONObject(query("money/info", new HashMap<String, String>(),2));
            USDFunds = BigDecimal.valueOf(jo.getJSONObject("data").getJSONObject("Wallets").getJSONObject("USD").getJSONObject("Balance").getDouble("value"));
        } catch (JSONException e) {
            utils.logger.log(true, e.getMessage());
    ...
    public void UpdateBTCFunds() {
        try {
            JSONObject jo = new JSONObject(query("money/info", new HashMap<String, String>(),2));
            BTCFunds = BigDecimal.valueOf(jo.getJSONObject("data").getJSONObject("Wallets").getJSONObject("BTC").getJSONObject("Balance").getDouble("value"));
        } catch (JSONException e) {
            utils.logger.log(true, e.getMessage());
    ...
    private String query(String path, HashMap<String, String> args, int apiVersion) {
        try {
            ...
            if (apiVersion == 1) {
                signature = Base64.encodeBase64String(mac.doFinal(post_data.getBytes()));
            } else if (apiVersion == 2) {
                signature = Base64.encodeBase64String(mac.doFinal((path + '\0' + post_data).getBytes()));
            }
    ...
    public void CancelOrder() {
        ...
        query("BTCUSD/money/order/cancel", args,2);
    }
}
